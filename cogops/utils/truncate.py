...
Tokenizer instances are cached per model_name — loading a HuggingFace
tokenizer is non-trivial and the same model is reused across every call.
"""
...
def truncate_messages_to_budget(
    ...
    if total_tokens(system_msgs + other_msgs) > max_tokens:
        ...
        for i in range(len(other_msgs) - 1, -1, -1):
            ...
            if content:
                remaining = max_tokens - total_tokens(system_msgs + other_msgs[:i])
                truncated = truncate_text_to_tokens(content, remaining, model_name)
